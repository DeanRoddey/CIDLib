...
namespace tCIDAI
{
    ...
    [[nodiscard]]  ENodeStates eXlatENodeStates(const TString& strToXlat, const tCIDLib::TBoolean bThrowIfNot = kCIDLib::False);
    [[nodiscard]]  const TString& strXlatENodeStates(const ENodeStates eToXlat, const tCIDLib::TBoolean bThrowIfNot = kCIDLib::True);
    [[nodiscard]]  tCIDLib::TBoolean bIsValidEnum(const ENodeStates eVal);

    inline TBinOutStream& operator<<(TBinOutStream& strmTar, const tCIDAI::ENodeStates eVal)
    {
        strmTar.WriteEnum(tCIDLib::TCard4(eVal));
        return strmTar;
    }
    inline TBinInStream& operator>>(TBinInStream& strmSrc, COP tCIDAI::ENodeStates& eToFill)
    {
        eToFill = tCIDAI::ENodeStates(strmSrc.c4ReadEnum());
        return strmSrc;
    }
