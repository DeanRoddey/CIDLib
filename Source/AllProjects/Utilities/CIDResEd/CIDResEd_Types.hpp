...
namespace tCIDResEd
{
    ...
    [[nodiscard]]  const TString& strAltXlatEMsgTypes(const EMsgTypes eToXlat, const tCIDLib::TBoolean bThrowIfNot = kCIDLib::True);
    [[nodiscard]]  EMsgTypes eAltXlatEMsgTypes(const TString& strToXlat, const tCIDLib::TBoolean bThrowIfNot = kCIDLib::False);
    [[nodiscard]]  tCIDLib::TBoolean bIsValidEnum(const EMsgTypes eVal);

    inline tCIDResEd::EMsgTypes operator++(CIOP tCIDResEd::EMsgTypes& eVal, int)
    {
        const tCIDResEd::EMsgTypes eTmp = eVal;
        eVal = tCIDResEd::EMsgTypes(int(eVal)+1);
        return eTmp;
    }

    inline TBinOutStream& operator<<(TBinOutStream& strmTar, const tCIDResEd::EMsgTypes eVal)
    {
        strmTar.WriteEnum(tCIDLib::TCard4(eVal));
        return strmTar;
    }
    inline TBinInStream& operator>>(TBinInStream& strmSrc, COP tCIDResEd::EMsgTypes& eToFill)
    {
        eToFill = tCIDResEd::EMsgTypes(strmSrc.c4ReadEnum());
        return strmSrc;
    }
    ...
    [[nodiscard]]  ETextSyms eXlatETextSyms(const TString& strToXlat, const tCIDLib::TBoolean bThrowIfNot = kCIDLib::False);
    [[nodiscard]]  const TString& strXlatETextSyms(const ETextSyms eToXlat, const tCIDLib::TBoolean bThrowIfNot = kCIDLib::True);
    [[nodiscard]]  tCIDLib::TBoolean bIsValidEnum(const ETextSyms eVal);

    inline tCIDResEd::ETextSyms operator++(CIOP tCIDResEd::ETextSyms& eVal, int)
    {
        const tCIDResEd::ETextSyms eTmp = eVal;
